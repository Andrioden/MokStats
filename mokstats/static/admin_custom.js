//console.log("Loading admin_custom.js script...");

// backgroundColor effect/animation plugin, probably Effects or part of Effects downloaded from http://jqueryui.com/download
// If jquery is upgraded, this should be upgraded as well and saved to its own file
...
/*
 * This javascript file contains several hacks and tweaks to the admin page for adding a match. These include:
 * - Adding a button to loading last player list via ajax.
 * - Trigger on change events to summarize row.
 * - Trigger on change events to possible fill in last players round sums.
 */
...
function summarizePlayerResult() {
    ...
    var total = 0;
    playerRow.find('input[type=number]').each(function(){
        var value = $(this).val();
        if (isNaN(value))
            alert("Not a number");
        else if (value != "") {
...
function autofillLastPlayerResultPossibly() {
    ...
    var playerRowInputs = [];
    $('table').first().find("select").each(function(i) {
        ...
        if (i < addedPlayersCount-1) { // Not last player row
            ...
            if (roundInputsWithValCount < 7)
                return false; // Stops loop
            else
                playerRowInputs.push(roundInputs);
        }
            ...
            // Spades
            if (!roundInputs.eq(0).val() && !changedInput.is(roundInputs.eq(0))) {
            ...
            // Queens
            if (!roundInputs.eq(1).val() && !changedInput.is(roundInputs.eq(1))) {
            ...
            $([4,5,6]).each(function(_, i) {
                if (!roundInputs.eq(i).val() && !changedInput.is(roundInputs.eq(i))) {
